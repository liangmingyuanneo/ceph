...
static int write_entry(cls_method_context_t hctx, T& entry, const string& key,
                       uint64_t gen = 0, const bool is_resharding = false)
{
  bufferlist bl;
  encode(entry, bl);
  ...
  if (is_resharding) {
    ret = reshard_log_index_operation(hctx, key, gen, entry.key, &bl);
  }
  return ret;
}
...
static int write_obj_instance_entry(cls_method_context_t hctx, rgw_bucket_dir_entry& instance_entry,
                                    const string& instance_idx, bool is_resharding, uint64_t gen)
{
  ...
  int ret = write_entry(hctx, instance_entry, instance_idx, gen, is_resharding);
  if (ret < 0) {
...
static int write_obj_entries(cls_method_context_t hctx, rgw_bucket_dir_entry& instance_entry,
                             const string& instance_idx, bool is_resharding, uint64_t gen)
{
  int ret = write_obj_instance_entry(hctx, instance_entry, instance_idx, is_resharding, gen);
  if (ret < 0) {
    return ret;
  }
  ...
  if (instance_idx != instance_list_idx) {
    ...
    ret = write_entry(hctx, instance_entry, instance_list_idx, gen, is_resharding);
    if (ret < 0) {
...
public:
  ...
  int unlink_list_entry(bool is_resharding, uint64_t gen) {
    ...
    if (is_resharding) {
      bufferlist empty;
      return reshard_log_index_operation(hctx, list_idx, gen, instance_entry.key, &empty);
    }
    return 0;
  }

  int unlink(bool is_resharding, uint64_t gen, const cls_rgw_obj_key& key) {
    ...
    if (is_resharding) {
      bufferlist empty;
      return reshard_log_index_operation(hctx, instance_idx, gen, key, &empty);
    }
    return 0;
  }

  int write_entries(uint64_t flags_set, uint64_t flags_reset, bool is_resharding, uint64_t gen) {
    if (!initialized) {
      int ret = init();
      if (ret < 0) {
        return ret;
    ...
    int ret = write_obj_entries(hctx, instance_entry, instance_idx, is_resharding, gen);
    if (ret < 0) {
  ...
  int write(uint64_t epoch, bool current, bool is_resharding, uint64_t gen) {
    if (instance_entry.versioned_epoch > 0) {
      ...
      /* this instance has a previous list entry, remove that entry */
      int ret = unlink_list_entry(is_resharding, gen);
      if (ret < 0) {
        return ret;
      }
    }
    ...
    instance_entry.versioned_epoch = epoch;
    return write_entries(flags, 0, is_resharding, gen);
  }

  int demote_current(bool is_resharding, uint64_t gen) {
    return write_entries(0, rgw_bucket_dir_entry::FLAG_CURRENT, is_resharding, gen);
  }
...
public:
  ...
  int write(bool is_resharding, uint64_t gen) {
    /* write the olh data entry */
    int ret = write_entry(hctx, olh_data_entry, olh_data_idx, gen, is_resharding);
    if (ret < 0) {
...
static int write_version_marker(cls_method_context_t hctx, cls_rgw_obj_key& key,
                                bool is_resharding, uint64_t gen)
{
  rgw_bucket_dir_entry entry;
  entry.key = key;
  entry.flags = rgw_bucket_dir_entry::FLAG_VER_MARKER;
  int ret = write_entry(hctx, entry, key.name, gen, is_resharding);
  if (ret < 0) {
                                            ...
                                            uint64_t gen)
{
  ...
  if (ret != -ENOENT) {
    ...
    if (instance_only) {
      ret = write_obj_instance_entry(hctx, entry, new_idx, is_resharding, gen);
    } else {
      ret = write_obj_entries(hctx, entry, new_idx, is_resharding, gen);
    }
    if (ret < 0) {
  ...
  ret = write_version_marker(hctx, key, is_resharding, gen);
  if (ret < 0) {
    return ret;
  }
...
static int rgw_bucket_link_olh(cls_method_context_t hctx, bufferlist *in, bufferlist *out)
{
  ...
  if (op.key.instance.empty()) {
    ...
    if (ret >= 0 && other_obj.is_delete_marker() != op.delete_marker) {
      ret = other_obj.unlink_list_entry(header.resharding_in_logrecord(), header.get_gen());
      if (ret < 0) {
        return ret;
      }
    }
    ...
    if (!removing) {
      ret = other_obj.unlink(header.resharding_in_logrecord(), header.get_gen(), op.key);
      if (ret < 0) {
        return ret;
      }
    }
  ...
  if (!olh.start_modify(op.olh_epoch)) {
    ret = obj.write(op.olh_epoch, false, header.resharding_in_logrecord(), header.get_gen());
    if (ret < 0) {
      return ret;
    }
    if (removing) {
  ...
  if (olh_found) {
    ...
    if (promote && olh.exists()) {
      ...
      if (!(olh_entry.key == op.key)) {
        ...
        ret = old_obj.demote_current(header.resharding_in_logrecord(), header.get_gen());
        if (ret < 0) {
    ...
    ret = convert_plain_entry_to_versioned(hctx, key, promote, instance_only,
                                           header.resharding_in_logrecord(), header.get_gen());
    if (ret < 0) {
  ...
  ret = olh.write(header.resharding_in_logrecord(), header.get_gen());
  if (ret < 0) {
  ...
  /* write the instance and list entries */
  ret = obj.write(olh.get_epoch(), promote, header.resharding_in_logrecord(), header.get_gen());
  if (ret < 0) {
    return ret;
  }
...
static int rgw_bucket_unlink_instance(cls_method_context_t hctx, bufferlist *in, bufferlist *out)
{
  ...
  if (!olh.start_modify(op.olh_epoch)) {
    ret = obj.unlink_list_entry(header.resharding_in_logrecord(), header.get_gen());
    if (ret < 0) {
      return ret;
    }
    ...
    olh.update_log(CLS_RGW_OLH_OP_REMOVE_INSTANCE, op.op_tag, op.key, false, op.olh_epoch);
    return olh.write(header.resharding_in_logrecord(), header.get_gen());
  }
  ...
  if (olh_key == dest_key) {
    ...
    if (found) {
      ...
      ret = next.write(olh.get_epoch(), true, header.resharding_in_logrecord(), header.get_gen());
      if (ret < 0) {
  ...
  if (!obj.is_delete_marker()) {
     ...
     * instance entry */
    ret = obj.unlink(header.resharding_in_logrecord(), header.get_gen(), op.key);
    if (ret < 0) {
      return ret;
    }
  }
...
static int rgw_bucket_trim_olh_log(cls_method_context_t hctx, bufferlist *in, bufferlist *out)
{
  ...
  /* write the olh data entry */
  ret = write_entry(hctx, olh_data_entry, olh_data_key, header.get_gen(),
...
static int rgw_bucket_clear_olh(cls_method_context_t hctx, bufferlist *in, bufferlist *out)
{
  ...
  if (header.resharding_in_logrecord()) {
    bufferlist empty;
    ret = reshard_log_index_operation(hctx, olh_data_key, header.get_gen(), olh_data_entry.key, &empty);
    if (ret < 0) {
  ...
  if (header.resharding_in_logrecord()) {
    bufferlist empty;
    ret = reshard_log_index_operation(hctx, op.key.name, header.get_gen(), plain_entry.key, &empty);
    if (ret < 0) {
  ...
  while (!in_iter.end()) {
	...
	if (ret < 0) {
	  ...
	  return ret;
        ...
        if (header.resharding_in_logrecord()) {
          bufferlist empty;
          return reshard_log_index_operation(hctx, cur_change_key, header.get_gen(), cur_change_key, &empty);
        }
        break;
	...
	CLS_LOG_BITX(bitx_inst, 20,
	  ...
	  return ret;
        ...
        if (header.resharding_in_logrecord()) {
          return reshard_log_index_operation(hctx, cur_change_key, header.get_gen(), cur_change.key, &cur_state_bl);
        }
        break;
      } // switch(op)
    } // if (cur_disk.pending_map.empty())
...
static int reshard_log_list_entries(cls_method_context_t hctx, const string& marker,
                                    uint64_t gen, uint32_t max,
                                    list<rgw_cls_bi_entry>& entries, bool *truncated)
{
  ...
  for (iter = keys.begin(); iter != keys.end(); ++iter) {
    ...
    rgw_cls_bi_entry entry;
    entry.idx = iter->first.substr(bi_type_marker.length());
...
static int rgw_reshard_log_list_op(cls_method_context_t hctx, bufferlist *in, bufferlist *out)
{
  ...
  rgw_cls_bi_list_ret op_ret;
  int ret = reshard_log_list_entries(hctx, op.marker, op.gen, op.max, op_ret.entries, &op_ret.is_truncated);
  if (ret < 0)
    return ret;
